function plot_pRF_results(subject,num_run_seq1,num_run_seq2,eyetracker)
% ----------------------------------------------------------------------
% plot_pRF_results(subject,numBlock,eyetracker)
% ----------------------------------------------------------------------
...
file_dir = sprintf('/Users/martin/Dropbox/Experiments/pRFseqTest/data/%s/func',subject);

task1_txt = 'AttendStim';
for task2 = 1:2
    if task2 == 1
        task2_txt = 'Seq1';
        num_run = num_run_seq1;
    elseif task == 2
        task2_txt = 'Seq2';
        num_run = num_run_seq2;
    end
    
    for t_run = 1:num_run
    
        % extract start time
        tsv_name = sprintf('%s/%s_task-%s%s_run-%i_events.tsv',file_dir,subject,task1_txt,task2_txt,t_run);
        val = tdfread(tsv_name);
        
        % transform to matlab nan
        for t_line = 1:size(val.response_val,1)
            if strcmp(val.response_val(t_line,:),'n/a');        response_val(t_line,:) = nan;
            else                                                response_val(t_line,:) = val.response_val(t_line,:);
            end
        end
        val.response_val = response_val;
        
        for t_line = 1:size(val.probe_time,1)
            if strcmp(val.probe_time(t_line,:),'n/a');          probe_time(t_line,:) = nan;
            else                                                probe_time(t_line,:) = val.probe_time(t_line,:);
            end
        end
        val.probe_time = probe_time;
        
        for t_line = 1:size(val.reaction_time,1)
            if strcmp(val.reaction_time(t_line,:),'n/a');       reaction_time(t_line,:) = nan;
            else                                                reaction_time(t_line,:) = val.reaction_time(t_line,:);
            end
        end
        val.reaction_time = reaction_time;
